...
type CrawlerResult<T> = Result<T, Box<dyn std::error::Error + Send + Sync>>;
...
pub trait CrawlerTrait {
    ...
    async fn download(
        &self,
        illu_id: &str,
    ) -> CrawlerResult<tauri::async_runtime::TokioJoinHandle<()>>;
    async fn set_total(&self, total: usize) -> CrawlerResult<()>;
    async fn add_finished(&self) -> CrawlerResult<()>;
    async fn add_failed(&self) -> CrawlerResult<()>;
    async fn process(&self) -> CrawlerResult<String>;
}
...
impl Crawler {
    pub fn new(uuid: &str, cookie: &str, proxy: &str, save_path: &str) -> Self {
        ...
        Self {
            ...
            cookie: cookie.into(),
            proxy,
            save_path: format!("{}/{}", save_path, uuid),
...
impl CrawlerTrait for Crawler {
    async fn run(&self) -> CrawlerResult<()> {
        // create save dir
        if !fs::try_exists(&self.save_path)
            .await
            .expect("Can not check if dir exists")
        {
        ...
        let client = Client::builder().proxy(self.proxy.clone()).build()?;
        let uuid = self.uuid.as_str();
        let url = format!("https://www.pixiv.net/ajax/user/{uuid}/profile/all?lang=zh");
        let resp = client.get(&url).send().await?;
        if let Ok(content) = resp.text().await {
            let json: serde_json::Value = serde_json::from_str(&content)?;
            let illus: Vec<String> = json.as_object().unwrap()["body"].as_object().unwrap()
                ["illusts"]
                .as_object()
                .unwrap()
                .keys()
                .map(|s| s.to_owned())
                .collect();
            self.set_total(illus.len())
                .await
                .expect("Can not set total number");

            let mut handles = Vec::new();
            for illu_id in illus.into_iter() {
                let handle = self.download(&illu_id).await.expect("download failed");
                handles.push(handle);
            }
    ...
    async fn download(
        ...
        let illu_id = illu_id.to_owned();
        let handle = tokio::spawn(async move {
            let info_url = format!("https://www.pixiv.net/ajax/illust/{illu_id}/pages?lang=zh");
            ...
            let resp = client
                ...
                .send()
                .await
                .expect("Can not ask illu info");
            if let Ok(content) = resp.text().await {
                let json: serde_json::Value =
                    serde_json::from_str(&content).expect("Can not serde json");
                let ori_urls: Vec<String> = json.as_object().unwrap()["body"]
                    .as_array()
                    .unwrap()
                    .iter()
                    .map(|i| {
                ...
                for url in ori_urls.into_iter() {
                    let client = Client::builder()
                        .proxy(proxy.clone())
                        .build()
                        .expect("Can not build client");
                    let resp = client.get(url).header("Cookie", cookie.clone())
                            ...
                            .header("Referer", format!("https://www.pixiv.net/artworks/{illu_id}"))
                            .send().await.expect(&format!("Server rejected {}", illu_id));
                    let path = format!("{}/{}.jpg", save_path, illu_id);
                    let mut file = OpenOptions::new()
                        .create(true)
                        .write(true)
                        .open(&path)
                        .await
                        .expect("Can not or create the jpg");
                    let content = resp.bytes().await.expect("Can not receive jpg content");
                    file.write_all(&content)
                        .await
                        .expect("Can not write into file");
                }
            }
        });
        self.add_finished().await.expect("Can not add");
        Ok(handle)
    }
    ...
    async fn add_finished(&self) -> CrawlerResult<()> {
        ...
        let process = self.process().await.unwrap();
        println!("{}", process);
        Ok(())
    }
...
mod tests {
    ...
    fn test_1960050() {
        ...
        let crawler = Crawler::new(uuid, cookie, proxy, &save_path);
        let rt = tokio::runtime::Builder::new_multi_thread()
            .enable_all()
            .build()
            .unwrap();
        let res = rt.block_on(crawler.run());
        assert!(res.is_ok());
    }
}
